...
{Fore.BLUE}╔══════════════════════════════════════════════════════════╗
║                                                              ║
║     {Fore.CYAN}TYBER ID – захват доменов...{Fore.BLUE}                      ║
╚══════════════════════════════════════════════════════════════╝{Style.RESET_ALL}
"""
    print(banner)
...
VK_VERSION = "5.199"
# Встроенный токен VK
VK_TOKEN = "vk1.a.uL1avUdx3GtZjaIffF96_yu8J-GyNY-lOltKB7qMTVREG4TJ0EdqdAREFYHu__3gIrUP9-R7oBCM-0s2dluESyXgXWfs_ZZdemi-g_bY_pzyWYJXMay8CB9nB67wVjU5qPXgr1jm52QhV2qS3IOq2F7pgh3hdwIMRd1-eV6WDfGjiI-gGDShFFr4Px1cFzw7aSR6oDJNaZJbBl6z2grGuAд"
...
def setup_telegram():
    ...
    if os.path.exists(TG_TOKEN_FILE):
        ...
        if input("Использовать сохранённый токен Telegram? (y/n): ").strip().lower() != 'y':
            tg_token = None
    if not tg_token:
        print_info("1. Открой @BotFather в Telegram, создай нового бота.")
...
def send_telegram(tg_token, chat_id, domain, success=True, old_owner=None, elapsed=0.0, target_desc="profile"):
    if not tg_token or not chat_id:
        return False
    if success:
        swap_info = f"Moved from: {old_owner} → {target_desc}" if old_owner else f"Moved from: None → {target_desc}"
        text = (f"✅ Domain Captured!\n\n"
                f"Domain: {domain}\n"
                f"{swap_info}\n"
                f"Status: Success\n"
...
def get_vk_token():
    # Сначала проверяем встроенный токен
    print_info("🔑 Проверка встроенного токена VK...")
    try:
        ...
        if 'error' in data:
            ...
            print_info("Пожалуйста, введите новый токен вручную.")
        else:
            user_id = data['response'][0]['id']
            print_success(f"✅ Встроенный токен VK работает! ID: {user_id}")
            
            # Сохраняем токен в файл для будущих запусков
            with open(TOKEN_FILE, "w", encoding='utf-8') as f:
                f.write(VK_TOKEN)
            
            # Спрашиваем, использовать ли встроенный токен
            if input("Использовать встроенный токен? (y/n): ").strip().lower() == 'y':
                return VK_TOKEN
    except Exception as e:
        print_error(f"Ошибка проверки встроенного токена: {e}")
    
    # Если встроенный токен не подошел или пользователь отказался, запрашиваем новый
    if os.path.exists(TOKEN_FILE):
        with open(TOKEN_FILE, "r", encoding='utf-8') as f:
            token = f.read().strip()
        if input("Использовать сохранённый токен VK из файла? (y/n): ").strip().lower() == 'y':
            return token
        ...
        while True:
            for domain in domains:
                ...
                if status is None:
                    continue
                if status == 'captcha':
                    print_error(f"❗️ Капча при проверке {domain}, жду {CAPTCHA_WAIT} сек...")
