...
Версия: интервал 10 мс, статистика запросов, повышенная отказоустойчивость
"""
...
# Таймауты увеличены для стабильности (было 0.3/0.4)
CHECK_TIMEOUT = 0.8          # для проверки домена
CAPTURE_TIMEOUT = 1.0        # для захвата домена
GROUP_TIMEOUT = 3.0          # для загрузки групп
...
def setup_telegram():
    ...
    if not chat_id:
        ...
        if send_telegram(tg_token, chat_id, "test", True, None, 0.0, "profile", 0, 0.0):
...
def send_telegram(tg_token, chat_id, domain, success=True, old_owner=None,
...
# ========== VK монитор с повышенной отказоустойчивостью ==========
class VKMonitor:
    def __init__(self, token):
        self.token = token
        self.session = requests.Session()
        retry_strategy = Retry(
            total=3,                          # три повторные попытки
    ...
    def _api_request(self, method, url, params=None, data=None, timeout=0.8, retries=2):
        params = params or {}
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if attempt < retries:
                    ...
                    return {'error': 'timeout' if isinstance(e, requests.exceptions.Timeout) else 'connection_error'}
            except Exception as e:
                return {'error': str(e)}
        return None
    ...
    def get_owner_info(self, domain):
        resp = self._api_request('GET', 'https://api.vk.com/method/utils.resolveScreenName',
                                 params={'screen_name': domain}, timeout=CHECK_TIMEOUT, retries=2)
        if resp is None:
            return None
        if resp == 'captcha':
            return 'captcha'
    ...
    def capture_domain(self, domain, target_type='profile', group_id=None):
        if target_type == 'profile':
            resp = self._api_request('POST', 'https://api.vk.com/method/account.saveProfileInfo',
                                     params={'screen_name': domain}, timeout=CAPTURE_TIMEOUT, retries=2)
        else:
            if not group_id:
            ...
            resp = self._api_request('POST', 'https://api.vk.com/method/groups.edit',
                                     params={'group_id': group_id, 'screen_name': domain}, timeout=CAPTURE_TIMEOUT, retries=2)
        if resp is None:
...
def main():
    print_banner()
    print_warning("⚠️ Интервал 10 мс, таймауты увеличены, повторные попытки — ошибок быть не должно.")
    ...
    if tg_token and tg_chatid:
        ...
        if send_telegram(tg_token, tg_chatid, "test", True, None, 0.0, target_desc, 0, 0.0):
