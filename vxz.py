...
"""
TYBER ID – захват доменов ВК + Telegram-уведомления (ультраскорость)
Версия: интервал проверки 10 мс, таргет 0.017-0.025 сек, статистика запросов
"""
...
║              {Fore.WHITE}{Style.BRIGHT}интервал 10 мс{Style.RESET_ALL}{Fore.BLUE}                          ║
║              {Fore.WHITE}{Style.BRIGHT}статистика в Telegram{Style.RESET_ALL}{Fore.BLUE}                   ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝{Style.RESET_ALL}
"""
    print(banner)
...
BASE_INTERVAL = 0.010        # 10 мс между циклами
CAPTCHA_WAIT = 15
MAX_CAPTURE_FAILS = 3

# Таймауты для целевого времени 0.017-0.025 сек
CHECK_TIMEOUT = 0.3          # для проверки домена
CAPTURE_TIMEOUT = 0.4        # для захвата домена
GROUP_TIMEOUT = 2.0          # для загрузки групп

# Встроенный токен VK (ты дал его)
...
def send_telegram(tg_token, chat_id, domain, success=True, old_owner=None, elapsed=0.0, target_desc="profile", total_requests=None):
    ...
    if success:
        ...
        text = f"Domain Supplied!\n\nDomain: {domain}\n{swap_info}\nStatus: Cut down\nCapture time: {elapsed:.3f} sec.\nTotal checks: {total_requests}\nEvent time: {time.strftime('%Y-%m-%d %H:%M:%S')}"
    else:
        text = f"⚠️ Capture error\n\nDomain: {domain}\nStatus: ❌ Failed\nReason: {old_owner if isinstance(old_owner, str) else 'unknown'}\nTotal checks: {total_requests}\nTime: {time.strftime('%Y-%m-%d %H:%M:%S')}"
    try:
...
# ========== VK монитор с повторными попытками ==========
class VKMonitor:
...
def main():
    print_banner()
    print_warning("⚠️ Интервал проверки 10 мс. Целевое время захвата 0.017-0.025 сек.")
                ...
                if status is True:
                    ...
                    if tg_token and tg_chatid:
                        send_telegram(tg_token, tg_chatid, domain, success,
                                      old_owner_str, elapsed, target_desc, total_requests)

                    if success:
                        print_success(f"✅ {domain} captured in {elapsed:.3f} sec! Total checks: {total_requests}")
                        input("\n🎉 Press Enter to exit...")
                        return
                    else:
...
# Сохраняем код в файл на рабочем столе
New-Item -Path "$env:USERPROFILE\Desktop\Tyber" -ItemType Directory -Force
$code | Out-File -FilePath "$env:USERPROFILE\Desktop\Tyber\tyber_id.py" -Encoding utf8
Write-Host "✅ Файл сохранён: $env:USERPROFILE\Desktop\Tyber\tyber_id.py"
