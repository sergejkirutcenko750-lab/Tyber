...
"""
TYBER ID – АСИНХРОННАЯ версия с диагностикой групп
Твой новый токен, все фишки сохранены
"""
...
║              {Fore.WHITE}{Style.BRIGHT}диагностика групп{Style.RESET_ALL}{Fore.BLUE}                       ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝{Style.RESET_ALL}
"""
    print(banner)
...
# Твой новый токен
VK_TOKEN = "vk1.a.bS2HjLHEfJEbErZjTxVA5HqFmpQXORgJUPtUvx6qToV_bWh4Yqz6cd6vq_lm37XMAW7U8Txl1pBzJYHoT5WUWjnA4IUOeCzykXBQ-V-4_tUR4sPVkX34stvC-jzJutq2jHDgNh5BEkoMWeo-m3OhvGZCsYtZ7wKAiBogF5rhVEwSLZr3JtlSoLtbxLA6fMoF6HT53baPZsMpn9qgiLT4JA"
...
async def vk_api_request(session, method, url, token, params=None, data=None, timeout=0.25, retries=1):
    params = params or {}
...
async def get_owner_info(session, token, domain):
    resp = await vk_api_request(session, 'GET', 'https://api.vk.com/method/utils.resolveScreenName',
                                 token, params={'screen_name': domain}, timeout=CHECK_TIMEOUT, retries=1)
    if resp is None:
        return None
    if resp == 'captcha':
        return 'captcha'
...
async def capture_domain(session, token, domain, target_type='profile', group_id=None):
    if target_type == 'profile':
        resp = await vk_api_request(session, 'POST', 'https://api.vk.com/method/account.saveProfileInfo',
                                     token, params={'screen_name': domain}, timeout=CAPTURE_TIMEOUT, retries=1)
    else:
        if not group_id:
        ...
        resp = await vk_api_request(session, 'POST', 'https://api.vk.com/method/groups.edit',
                                     token, params={'group_id': group_id, 'screen_name': domain}, timeout=CAPTURE_TIMEOUT, retries=1)
    if resp is None:
...
# ========== Функция загрузки групп с диагностикой ==========
async def load_groups(session, token):
    ...
    try:
        ...
        print("=== ДИАГНОСТИКА ЗАГРУЗКИ ГРУПП ===")
        ...
        if 'error' in data:
            ...
            print_info("1. Токен не имеет прав groups (получи новый на vkhost.github.io с галочкой groups)")
            print_info("2. Токен истёк или недействителен")
            print_info("3. У тебя нет групп, где ты администратор")
            return []
...
async def main_async():
    print_banner()
    print_warning("⚠️ Таймауты 0.25/0.3, диагностика групп включена.")
    ...
    async with aiohttp.ClientSession() as session:
        # Загружаем группы с диагностикой
