...
"""
TYBER ID – АСИНХРОННАЯ версия для 20 RPS (пинг 10 мс)
Таргет: 0.02–0.05 сек, встроенный токен, статистика в Telegram
"""
...
║              {Fore.WHITE}{Style.BRIGHT}20 RPS | пинг 10 мс{Style.RESET_ALL}{Fore.BLUE}                     ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝{Style.RESET_ALL}
"""
    print(banner)
...
# Агрессивные таймауты для пинга 10 мс
CHECK_TIMEOUT = 0.15
CAPTURE_TIMEOUT = 0.2
GROUP_TIMEOUT = 2.0
...
async def vk_api_request(session, method, url, token, params=None, data=None, timeout=0.15, retries=1):
    params = params or {}
...
async def get_owner_info(session, token, domain):
    ...
    if isinstance(resp, dict) and 'error_code' in resp:
        # Не выводим ошибку, чтобы не засорять логи
        return None
...
async def main_async():
    print_banner()
    print_warning("⚠️ Асинхронная версия, таймауты 0.15/0.2, цель 20 RPS, захват 0.02-0.05 сек.")
    ...
    async with aiohttp.ClientSession() as session:
        target_type, group_id = await choose_target(session, vk_token)
